...
export default async function (instance: FastifyInstance, opts: FastifyServerOptions, done) {
	...
	instance.register(async (instance: FastifyInstance, opts: FastifyServerOptions, done) => {
		...
		instance.get<{
			Querystring: IQueryString
		}>('/', async (req: FastifyRequest<CustomRouteGenericQuery>, res: FastifyReply) => {
		...
		instance.get<{
			Querystring: IQueryString,
			Params: IParams
		}>('/:name', async (req: FastifyRequest<CustomRouteGenericParam>, res: FastifyReply) => {
	...
	instance.register(import('./order-data'))
